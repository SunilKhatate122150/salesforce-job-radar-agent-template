...
var TOPIC_DATA = {
  ...
  'apex': {
    title: 'Apex Core — Interview Questions',
    subtitle: '22 high-impact questions covering basics to complex logic',
    blocks: [
      { type: 'section', title: '🚀 Fundamentals & OOP' },
      { type: 'qa', question: 'What is Apex? What are its key features?', answer: 'Apex is a strongly typed, object-oriented programming language that allows developers to execute flow and transaction control statements on the Salesforce platform. Features: Integrated with the database, easy to use, strongly typed, multitenant aware, and automatically versioned.' },
      { type: 'qa', question: 'Difference between Public, Private, Protected, and Global access modifiers?', answer: 'Private: accessible only within the class. Public: accessible within the namespace. Protected: accessible to inner classes and subclasses. Global: accessible across namespaces (use sparingly for managed packages).' },
      { type: 'qa', question: 'What is "this" keyword in Apex?', answer: 'Used to refer to the current instance of a class, useful for resolving naming conflicts between instance variables and parameters.' }
    ]
  },
  'soql': {
    title: 'SOQL Deep Dive — Interview Questions',
    subtitle: '14 questions with complete answers and code examples',
    blocks: [
      { type: 'section', title: '🔍 Query Fundamentals' },
      { type: 'qa', question: 'What is SOQL injection and how do you prevent it?', answer: 'SOQL injection is a vulnerability where malicious input manipulates dynamic queries. Prevention: Use static SOQL, bind variables (:var), or String.escapeSingleQuotes().' },
      { type: 'qa', question: 'Explain Child-to-Parent vs Parent-to-Child queries.', answer: 'Child-to-Parent: Use dot notation (Contact.Account.Name). Parent-to-Child: Use subqueries (SELECT Name, (SELECT LastName FROM Contacts) FROM Account).' }
    ]
  },
  'triggers': {
    title: 'Triggers & Handler Patterns',
    subtitle: 'Advanced routing and business logic separation',
    blocks: [
      { type: 'section', title: '⚡ Trigger Architecture' },
      { type: 'qa', question: 'Why use a Trigger Handler pattern?', answer: 'It ensures separation of concerns (trigger = routing, handler = logic), allows for easy unit testing, and facilitates bypass logic for data migrations.' },
      { type: 'qa', question: 'How do you prevent recursion in triggers?', answer: 'Use a static Boolean flag (isFirstRun) or a static Set of processed record IDs to track what has already been handled in the transaction.' }
    ]
  },
  'async': {
    title: 'Async Apex — Interview Questions',
    subtitle: 'Future, Queueable, Batch, and Scheduled logic',
    blocks: [
      { type: 'section', title: '🕒 Asynchronous Patterns' },
      { type: 'qa', question: 'Future vs Queueable?', answer: 'Future: Simple, primitive params only, not chainable. Queueable: Supports sObjects, chainable (one level), and returns a Job Id for monitoring.' },
      { type: 'qa', question: 'What is a Mixed DML error?', answer: 'Occurs when DML is performed on setup (User/Profile) and non-setup (Account/Loan) objects in the same transaction. Fix: Use @future for the non-setup DML.' }
    ]
  },
  'lwc': {
    title: 'Lightning Web Components (LWC)',
    subtitle: 'Modern web standards for Salesforce UI development',
    blocks: [
      { type: 'section', title: '🏗️ LWC Core' },
      { type: 'qa', question: 'Decorators: @api, @track, @wire?', answer: '@api: Public property for parent-child comms. @track: Deep reactivity for objects/arrays. @wire: Declarative data fetching (cached).' },
      { type: 'qa', question: 'LWC Lifecycle Hooks?', answer: 'constructor -> connectedCallback -> render -> renderedCallback -> disconnectedCallback. errorCallback for error boundaries.' }
    ]
  },
  'integration': {
    title: 'Integration & APIs',
    subtitle: 'Connecting Salesforce to the external world',
    blocks: [
      { type: 'section', title: '🌐 API Patterns' },
      { type: 'qa', question: 'What are Named Credentials?', answer: 'A secure way to store callout endpoints and authentication details. It abstracts credentials from code and handles OAuth handshakes automatically.' },
      { type: 'qa', question: 'REST vs SOAP?', answer: 'REST: Lightweight, uses JSON/XML, stateless, easier to consume. SOAP: Heavier, XML only, has a strict WSDL contract, better for formal enterprise APIs.' }
    ]
  },
  'fde_ag_concept': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'What are the 5 components of Agentforce?', answer: 'Agent, Topics, Actions, Atlas, and Trust Layer.' }
    ]
  },
  'fde_atlas': {
  ...
  'comm30': {
    title: '30-Day Communication Plan',
    subtitle: 'Daily structured exercises to build voice, pace, and clarity',
    blocks: [
      { type: 'section', title: '📅 Week 1: Foundation' },
      { type: 'qa', question: 'Day 1: Baseline Intro', answer: 'Record a 2-minute video of your self-introduction. Identify fillers like "um" or "basically".' }
    ]
  },
  'domain': {
    title: 'US Mortgage Domain — Interview Prep',
    subtitle: '15 questions on Loan Origination, Servicing, and Compliance',
    blocks: [
      { type: 'section', title: '🏠 Mortgage Lifecycle' },
      { type: 'qa', question: 'Explain the Loan Origination Process (LOS).', answer: '1. Lead/Inquiry -> 2. Application (1003 form) -> 3. Processing -> 4. Underwriting -> 5. Closing -> 6. Funding.' },
      { type: 'qa', question: 'What is a 1003 Form?', answer: 'The Uniform Residential Loan Application (URLA) used by lenders to determine a borrower\'s eligibility.' },
      { type: 'qa', question: 'Difference between Pre-Qualification and Pre-Approval?', answer: 'Pre-Qual is based on unverified data. Pre-Approval is a formal commitment after credit/income verification.' }
    ]
  },
  'security': {
    title: 'Security & Sharing — Advanced',
    subtitle: 'FLS, OWD, and complex sharing scenarios',
    blocks: [
      { type: 'section', title: '🛡️ Security Mastery' },
      { type: 'qa', question: 'How to handle security in Apex?', answer: 'Use "with sharing" to respect sharing rules. Use "Security.stripInaccessible" or "WITH USER_MODE" to respect FLS.' }
    ]
  },
  'design': {
    title: 'Design Patterns in Apex',
    subtitle: 'Singleton, Strategy, and Unit of Work',
    blocks: [
      { type: 'section', title: '🏛️ Architecture Patterns' },
      { type: 'qa', question: 'Explain the Unit of Work pattern.', answer: 'It collects all DML operations in a single transaction to ensure "all or nothing" success and save on limits.' }
    ]
  },
  'adv_apex': {
    title: 'Advanced Apex Architecture',
    subtitle: 'Senior-level performance and scaling questions',
    blocks: [
      { type: 'section', title: '🚀 Scaling & Logic' },
      { type: 'qa', question: 'How to optimize SOQL in triggers?', answer: 'Use Maps for O(1) lookups, avoid nested loops, and ensure selective queries using indexes.' }
    ]
  },
  'adv_lwc': {
    title: 'Advanced LWC — Performance',
    subtitle: 'Jest, LMS, and performance tuning',
    blocks: [
      { type: 'section', title: '🏗️ UI Engineering' },
      { type: 'qa', question: 'How to handle large data in LWC?', answer: 'Use lazy loading, server-side pagination, and lightning-datatable with infinite loading.' }
    ]
  },
  'admin': {
    title: 'Admin & Configuration',
    subtitle: 'Advanced config, Flows, and Validation',
    blocks: [
      { type: 'section', title: '⚙️ Config Mastery' },
      { type: 'qa', question: 'Flow vs Workflow vs Process Builder?', answer: 'Flow is the only tool moving forward. It supports complex logic, screens, and before-save performance.' }
    ]
  },
  'scenario': {
    title: 'Scenario-Based Technical Drill',
    subtitle: 'Real-world problem solving for Senior Devs',
    blocks: [
      { type: 'section', title: '🧠 Scenario Solving' },
      { type: 'qa', question: 'Scenario: 1M records update without batch?', answer: 'Use Queueable with chaining or Platform Events to process in manageable chunks.' }
    ]
  },
  'agentforce': {
    title: 'Agentforce Architecture',
    subtitle: 'Autonomous agents and reasoning loops',
    blocks: [
      { type: 'section', title: '🤖 AI Strategy' },
      { type: 'qa', question: 'What is the Atlas Reasoning Engine?', answer: 'The brain of Agentforce that plans and executes actions based on the user\'s prompt and available metadata.' }
    ]
  },
  'flows': {
    title: 'Salesforce Flow Guide',
    subtitle: 'Record-triggered, Screen, and Autolaunched flows',
    blocks: [
      { type: 'section', title: '🌊 Flow Design' },
      { type: 'qa', question: 'Before-Save vs After-Save Flow?', answer: 'Before-save is 10x faster for field updates on the same record. After-save is for cross-object updates.' }
    ]
  },
  'reports': {
    title: 'Reports & Dashboards',
    subtitle: 'Analytics and complex reporting scenarios',
    blocks: [
      { type: 'section', title: '📊 Analytics' },
      { type: 'qa', question: 'What is a Joined Report?', answer: 'Combines multiple report types in a single view, sharing common fields like Account Name.' }
    ]
  },
  'salary': {
    title: 'Salary & Negotiation',
    subtitle: 'Scripts and strategies for senior roles',
    blocks: [
      { type: 'section', title: '💰 Negotiation' },
      { type: 'qa', question: 'How to handle "What is your current salary?"', answer: '"I\'m focusing on the value I bring and the market rate for this role, which is [Range]."' }
    ]
  },
  'questions': {
    title: 'Questions to Ask Interviewers',
    subtitle: 'Reverse interviewing for technical teams',
    blocks: [
      { type: 'section', title: '🧐 Due Diligence' },
      { type: 'qa', question: 'Question about Tech Debt?', answer: '"How does the team balance new feature development with technical debt and architectural improvements?"' }
    ]
  },
  'mock': {
    title: 'Mock Interview Prep',
    subtitle: 'Scripts for Round 1 and Round 2',
    blocks: [
      { type: 'section', title: '🎙️ Mock Script' },
      { type: 'qa', question: 'Round 1 Focus?', answer: 'Core technical competency, coding logic, and cultural fit.' }
    ]
  },
  'mobigic_pwc': {
  ...
  'apex': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'What is Apex? What are its key features?', answer: 'Apex is a strongly typed, object-oriented programming language that allows developers to execute flow and transaction control statements on the Salesforce platform. Features: Integrated with the database, easy to use, strongly typed, multitenant aware, and automatically versioned.' },
      { type: 'qa', question: 'Difference between Public, Private, Protected, and Global access modifiers?', answer: 'Private: accessible only within the class. Public: accessible within the namespace. Protected: accessible to inner classes and subclasses. Global: accessible across namespaces (use sparingly for managed packages).' },
      { type: 'section', title: '🛠️ Logic & Data Management' },
      { type: 'qa', question: 'What is the difference between a Map and a Set?', answer: 'Set: Unordered collection of unique elements. Map: Collection of key-value pairs where each key is unique and maps to a specific value.' },
      { type: 'qa', question: 'How do you handle exceptions in Apex?', answer: 'Use try-catch-finally blocks. Always catch specific exceptions (like DmlException, QueryException) before generic Exception.' },
      { type: 'qa', question: 'What are Governor Limits? Why do they exist?', answer: 'Governor limits are runtime limits enforced by the Apex engine to ensure that multi-tenant resources are not monopolized by a single org. Examples: 100 SOQL queries, 150 DML statements, 10,000 max records per transaction.' }
    ]
  },
  'soql': {
    title: 'SOQL Deep Dive — Interview Questions',
    ...
    blocks: [
      ...
      { type: 'qa', question: 'What is SOQL injection and how do you prevent it?', answer: 'SOQL injection is a vulnerability where malicious input manipulates dynamic queries. Prevention: Use static SOQL, bind variables (:var), or String.escapeSingleQuotes().' },
      { type: 'qa', question: 'Explain Child-to-Parent vs Parent-to-Child queries.', answer: 'Child-to-Parent: Use dot notation (Contact.Account.Name). Parent-to-Child: Use subqueries (SELECT Name, (SELECT LastName FROM Contacts) FROM Account).' },
      { type: 'qa', question: 'What is AggregateResult?', answer: 'It is a special object used for SOQL aggregate functions like COUNT(), SUM(), AVG(). Access fields using ar.get(\'alias\').' },
      { type: 'qa', question: 'Difference between WITH SECURITY_ENFORCED and WITH USER_MODE?', answer: 'WITH SECURITY_ENFORCED throws an error if any field is inaccessible. WITH USER_MODE (newer) strips fields silently and supports DML/subqueries better.' }
    ]
  },
  'triggers': {
    title: 'Triggers & Handler Patterns',
    ...
    blocks: [
      ...
      { type: 'qa', question: 'Why use a Trigger Handler pattern?', answer: 'It ensures separation of concerns (trigger = routing, handler = logic), allows for easy unit testing, and facilitates bypass logic for data migrations.' },
      { type: 'qa', question: 'How do you prevent recursion in triggers?', answer: 'Use a static Boolean flag (isFirstRun) or a static Set of processed record IDs to track what has already been handled in the transaction.' },
      { type: 'qa', question: 'Before vs After Triggers?', answer: 'Before: Use for validation and field updates (no DML needed). After: Use for cross-object updates or when you need the record Id (like after insert).' }
    ]
  },
  'async': {
    title: 'Async Apex — Interview Questions',
    ...
    blocks: [
      ...
      { type: 'qa', question: 'Future vs Queueable?', answer: 'Future: Simple, primitive params only, not chainable. Queueable: Supports sObjects, chainable (one level), and returns a Job Id for monitoring.' },
      { type: 'qa', question: 'What is a Mixed DML error?', answer: 'Occurs when DML is performed on setup (User/Profile) and non-setup (Account/Loan) objects in the same transaction. Fix: Use @future for the non-setup DML.' },
      { type: 'qa', question: 'Batch Apex: Database.Stateful?', answer: 'By default, instance variables reset between batch chunks. Database.Stateful preserves variable state across all execute() calls.' }
    ]
  },
  'lwc': {
    title: 'Lightning Web Components (LWC)',
    ...
    blocks: [
      ...
      { type: 'qa', question: 'Decorators: @api, @track, @wire?', answer: '@api: Public property for parent-child comms. @track: Deep reactivity for objects/arrays. @wire: Declarative data fetching (cached).' },
      { type: 'qa', question: 'LWC Lifecycle Hooks?', answer: 'constructor -> connectedCallback -> render -> renderedCallback -> disconnectedCallback. errorCallback for error boundaries.' },
      { type: 'qa', question: 'LWC vs Aura?', answer: 'LWC is based on modern web standards (ES6+), faster, easier to test, and more secure via Shadow DOM. Aura is legacy and proprietary.' }
    ]
  },
  'integration': {
    title: 'Integration & APIs',
    ...
    blocks: [
      ...
      { type: 'qa', question: 'What are Named Credentials?', answer: 'A secure way to store callout endpoints and authentication details. It abstracts credentials from code and handles OAuth handshakes automatically.' },
      { type: 'qa', question: 'REST vs SOAP?', answer: 'REST: Lightweight, uses JSON/XML, stateless, easier to consume. SOAP: Heavier, XML only, has a strict WSDL contract, better for formal enterprise APIs.' },
      { type: 'qa', question: 'How to handle callout timeouts?', answer: 'Use req.setTimeout(ms). Maximum limit is 120,000ms (2 minutes). Always wrap in try-catch for CalloutException.' }
    ]
  },
  'security': {
    title: 'Security & Sharing Model',
    subtitle: 'Protecting data from Org level to Record level',
    blocks: [
      { type: 'section', title: '🛡️ Security Layers' },
      { type: 'qa', question: 'What is OWD (Org-Wide Defaults)?', answer: 'The baseline level of record access (Private, Public Read, Public Read/Write). You set OWD to most restrictive and open up using sharing rules.' },
      { type: 'qa', question: 'With Sharing vs Without Sharing?', answer: 'With Sharing: Enforces the running user\'s sharing rules. Without Sharing: Runs in system mode, ignoring sharing rules.' },
      { type: 'qa', question: 'How to enforce FLS in Apex?', answer: 'Use WITH USER_MODE in SOQL, or Security.stripInaccessible() for DML operations to respect field-level permissions.' }
    ]
  },
  'design': {
    title: 'Design Patterns in Apex',
    subtitle: 'Writing scalable and maintainable enterprise code',
    blocks: [
      { type: 'section', title: '🏛️ Architecture Patterns' },
      { type: 'qa', question: 'What is the Strategy Pattern?', answer: 'Defining a family of algorithms, encapsulating each, and making them interchangeable. Example: Different approval logic for different loan types.' },
      { type: 'qa', question: 'What is the Singleton Pattern?', answer: 'Ensuring a class has only one instance and providing a global point of access. Used for caching configurations to save SOQL limits.' },
      { type: 'qa', question: 'Unit of Work Pattern?', answer: 'Collecting all pending DML operations and executing them in a single batch to minimize DML statements and ensure transactional integrity.' }
    ]
  },
  'adv_apex': {
    title: 'Advanced Apex — Senior Level',
    subtitle: '18 high-impact questions on performance, patterns, and complex logic',
    blocks: [
      { type: 'section', title: '🚀 Performance & Scaling' },
      { type: 'qa', question: 'How do you optimize a CPU Time Limit Exceeded error?', answer: 'Use Maps for O(1) lookups instead of nested loops. Avoid string concatenation in loops (use List.join()). Only query fields you need. Offload complex logic to @future or Queueable.' },
      { type: 'qa', question: 'What is Apex Managed Sharing?', answer: 'Programmatic record sharing using a custom RowCause on the __Share object. It persists across ownership changes and allows for complex, dynamic access logic.' },
      { type: 'section', title: '🛠️ Complex Patterns' },
      { type: 'qa', question: 'Queueable chaining vs Batch Apex?', answer: 'Batch: Best for 50k+ records, processing in chunks. Queueable: Best for sequential logic (chaining), callouts, and smaller async tasks that need sObject support.' }
    ]
  },
  'adv_lwc': {
    title: 'Advanced LWC — Senior Level',
    subtitle: 'Deeper questions on performance, architecture, and testing',
    blocks: [
      { type: 'section', title: '🏗️ Architecture & Testing' },
      { type: 'qa', question: 'How to optimize slow LWC components?', answer: 'Lazy load data using imperative calls. Cache computed getters to avoid re-renders. Use server-side pagination for large tables. Unsubscribe LMS in disconnectedCallback.' },
      { type: 'qa', question: 'Jest Testing: Mocking Apex?', answer: 'Use jest.mock() to virtualize the Apex module. Use createApexTestWireAdapter() from @salesforce/sfdx-lwc-jest to emit data/error to the wire.' },
      { type: 'qa', question: 'Shadow DOM vs CSS Scoping?', answer: 'Shadow DOM encapsulates styles. Use :host to style the root. Use CSS custom properties (variables) to pass styles across shadow boundaries.' }
    ]
  },
  'adv_intg': {
    title: 'Advanced Integration — Senior Level',
    subtitle: 'Real enterprise integration patterns and retry logic',
    blocks: [
      { type: 'section', title: '🌐 Enterprise Patterns' },
      { type: 'qa', question: 'How to implement Retry Logic in Apex?', answer: 'Use a loop with a counter for simple retries (with sleep). Better: use a custom object to queue failures and a Scheduled Job to retry with exponential backoff.' },
      { type: 'qa', question: 'REST API: Custom Apex Endpoint?', answer: 'Use @RestResource(urlMapping=\'/...\') and @HttpGet/@HttpPost. Use RestContext.request and RestContext.response to handle input/output.' }
    ]
  },
  'admin': {
    title: 'Objects & Fields — Configuration Scenarios',
    subtitle: '10 real-world scenarios on Salesforce data modeling and schema design',
    blocks: [
      { type: 'section', title: '🏗️ Schema & Relationships' },
      { type: 'qa', question: 'Lookup vs Master-Detail: When to use which?', answer: 'Master-Detail: Parent required, cascade delete, rollup summaries, child inherits sharing. Lookup: Parent optional, independent sharing, no rollups. Use M-D for strong ownership, Lookup for loose connections.' },
      { type: 'qa', question: 'What is a Junction Object?', answer: 'A custom object with two Master-Detail relationships, used to create a many-to-many relationship between two objects. Example: Loan_Borrower__c connects many Loans to many Contacts.' },
      { type: 'qa', question: 'How to replicate Rollup Summary on Lookup?', answer: 'Use an Apex Trigger on the child object (insert, update, delete, undelete) to aggregate the counts/sums and update the parent record.' }
    ]
  },
  'scenario': {
    title: 'Record Page & UI Scenarios',
    subtitle: 'LWC, LDS, and LMS communication on standard record pages',
    blocks: [
      { type: 'section', title: '💻 Component Communication' },
      { type: 'qa', question: 'How do sibling components on a record page communicate?', answer: 'Use Lightning Message Service (LMS). It allows cross-component communication even if components are not in the same DOM hierarchy.' },
      { type: 'qa', question: 'LDS vs Apex: Performance choice?', answer: 'Use Lightning Data Service (LDS) for single record CRUD to benefit from client-side caching and shared data across components. Use Apex for complex queries or business logic.' }
    ]
  },
  'comm30': {
    title: '30-Day Communication Plan',
    ...
    blocks: [
      { type: 'section', title: '📅 Week 1: Foundation' },
      { type: 'qa', question: 'Day 1: Baseline Intro', answer: 'Record a 2-minute video of your self-introduction. Identify fillers like "um" or "basically". This is your starting point.' },
      { type: 'qa', question: 'Day 4: Explain to Non-Tech', answer: 'Explain a technical concept (like Governor Limits) to a non-technical person using analogies. This trains clarity of thought.' },
      { type: 'section', title: '📅 Week 2: Structure' },
      { type: 'qa', question: 'Day 8: PREP Framework', answer: 'Point -> Reason -> Example -> Point. Use this for every technical answer to sound senior and decisive.' }
    ]
  },
  'speaking': {
    title: 'Speaking Drills & Exercises',
    subtitle: '40 exercises to polish pronunciation, stress, and confidence',
    blocks: [
      { type: 'section', title: '🗣️ Pronunciation Drills' },
      { type: 'qa', question: 'Technical Word Stress', answer: 'Asynchronous (a-SIN-kro-nus), Idempotent (eye-DEM-po-tent), Abstraction (ab-STRAK-shun). Practice each 5 times slowly.' },
      { type: 'qa', question: 'The Power Pause', answer: 'Take a 2-3 second pause after being asked a question. It signals confidence and gives you time to structure your PREP response.' }
    ]
  },
  'mistakes': {
    title: 'Common Communication Mistakes',
    subtitle: 'Patterns to avoid to sound like a Senior Architect',
    blocks: [
      { type: 'section', title: '⚠️ Critical Fixes' },
      { type: 'qa', question: 'Mistake: Saying "We" instead of "I"', answer: 'Interviewers hire individuals, not teams. Own your decisions and code. Say "I designed the integration" instead of "We did it".' },
      { type: 'qa', question: 'Mistake: Trailing off at the end', answer: 'Avoid ending with "...yeah" or "...basically". Close decisively: "That is the approach I follow in production."' }
    ]
  },
  'agentforce': {
    title: 'Agentforce & AI Architecture',
    subtitle: 'Autonomous AI agents, Trust Layer, and Apex Actions',
    blocks: [
      { type: 'section', title: '🤖 Autonomous Agents' },
      { type: 'qa', question: 'Agentforce vs Einstein Copilot?', answer: 'Agentforce is the 2025 evolution. Copilot was human-in-the-loop; Agentforce is autonomous, acting on events and schedules across all channels (Email, Slack, Web).' },
      { type: 'qa', question: 'How to build an Apex Action for Agentforce?', answer: 'Use @InvocableMethod. The key is the Description field—it tells the LLM WHEN to use the action. Always use WITH USER_MODE to respect security.' }
    ]
  },
  'flows': {
    title: 'Salesforce Flow — Developer Guide',
    subtitle: 'All 6 flow types, integration patterns, and decision frameworks',
    blocks: [
      { type: 'section', title: '⚡ Flow Architecture' },
      { type: 'qa', question: 'When to use Flow vs Apex?', answer: 'Use Flow for simple logic, email alerts, and single-record updates. Use Apex for bulk operations (200+), complex loops/maps, and external callouts.' },
      { type: 'qa', question: 'How to debug a failing Flow?', answer: 'Use the "Debug" button in Flow Builder to see path execution. For production failures, check Flow Error Emails and Fault Paths.' }
    ]
  },
  'reports': {
    title: 'Reports & Dashboards — Analytics',
    subtitle: 'Summary, Matrix, Joined reports and Apex Analytics API',
    blocks: [
      { type: 'section', title: '📊 Data Visualization' },
      { type: 'qa', question: 'Summary vs Matrix reports?', answer: 'Summary groups rows (e.g., Loans by Status). Matrix groups both rows and columns (e.g., Loans by Status AND Month).' },
      { type: 'qa', question: 'How to run a report from Apex?', answer: 'Use Reports.ReportManager.runReport(reportId, true). Use the getFactMap() to access data cells and totals.' }
    ]
  },
  'eng_starters': {
    title: '50 Sentence Starters',
    subtitle: 'Professional phrases to lead technical and behavioral answers',
    blocks: [
      { type: 'section', title: '🚀 Opening Starters' },
      { type: 'qa', question: 'Starting a Technical Answer', answer: '"The way I think about this is...", "In my experience building mortgage systems...", "Let me walk you through how this works in practice..."' },
      { type: 'qa', question: 'Explaining a Tradeoff', answer: '"It depends on the specific situation...", "Both approaches work, but the tradeoff is...", "My preference is [X] because [Reason]..."' }
    ]
  },
  'eng_phrases': {
    title: 'Difficult Interview Situations',
    subtitle: 'Scripts for when you are challenged or unsure',
    blocks: [
      { type: 'section', title: '🛡️ Recovery Scripts' },
      { type: 'qa', question: 'When you do not know the answer', answer: '"I have not worked with that specific feature in production. Let me reason through it: based on [Concept], I would expect..."' },
      { type: 'qa', question: 'Handling a Challenge', answer: '"That is a fair point—I may have oversimplified. Let me be more specific: [Detail]..."' }
    ]
  },
  'platform': {
    title: 'Platform Events & Event-Driven Architecture',
    subtitle: 'Streaming API, CDC, and decoupled system communication',
    blocks: [
      { type: 'section', title: '📡 Event Messaging' },
      { type: 'qa', question: 'What are Platform Events?', answer: 'Schematized events for event-driven architecture. Use them to decouple processes (e.g., Loan Approval triggers DocuSign via event). Subscriptions are via Apex Trigger, Flow, or LWC.' },
      { type: 'qa', question: 'CDC vs Platform Events?', answer: 'CDC is auto-generated by Salesforce on record DML. Platform Events are custom events you explicitly publish. CDC is best for data sync; PE is best for business process events.' }
    ]
  },
  'security': {
    title: 'Security & Sharing — 5-Layer Model',
    subtitle: 'OWD, Role Hierarchy, Sharing Rules, and FLS enforcement',
    blocks: [
      { type: 'section', title: '🔐 Data Security' },
      { type: 'qa', question: 'Explain the 4 gates of security.', answer: '1. Org (IP/Hours) -> 2. Object (CRUD) -> 3. Field (FLS) -> 4. Record (Sharing). A user must pass all 4 gates to access specific data.' },
      { type: 'qa', question: 'with sharing vs without sharing?', answer: 'with sharing: Enforces user sharing rules. without sharing: Runs in system mode (ignores sharing). Always use with sharing unless there is a specific system-level reason not to.' }
    ]
  },
  'design': {
    title: 'Design Patterns in Apex',
    subtitle: 'Strategy, Service Layer, Factory, and Singleton patterns',
    blocks: [
      { type: 'section', title: '🏗️ Architectural Patterns' },
      { type: 'qa', question: 'Explain the Strategy Pattern.', answer: 'Defines a family of interchangeable algorithms. Example: Different loan types (FHA, VA) having separate approval logic classes implementing a common interface.' },
      { type: 'qa', question: 'What is a Singleton in Apex?', answer: 'Ensures a class has only one instance per transaction. Critical for caching configuration or metadata to avoid hitting SOQL governor limits in loops.' }
    ]
  },
  'adv_apex': {
    title: 'Advanced Apex — Senior Masterclass',
    subtitle: 'CPU limits, Stateful Batch, Managed Sharing, and Chaining',
    blocks: [
      { type: 'section', title: '🚀 Performance & Scaling' },
      { type: 'qa', question: 'Database.Stateful vs default Batch?', answer: 'Default Batch resets variables between chunks. Database.Stateful preserves instance variables across all execute() calls—essential for counters, error lists, or aggregators.' },
      { type: 'qa', question: 'How to reduce Apex CPU time?', answer: 'Avoid string concatenation in loops (use List + join), move nested loops to Maps (O(n) instead of O(n²)), and compile RegEx once outside loops.' }
    ]
  },
  'adv_lwc': {
    title: 'Advanced LWC — Senior Architecture',
    subtitle: 'Shadow DOM, Jest Testing, LDS Cache, and LMS',
    blocks: [
      { type: 'section', title: '🏗️ Component Architecture' },
      { type: 'qa', question: 'Shadow DOM css scoping?', answer: 'CSS inside a component does not leak out. Use :host to style the root, and CSS Variables to pass styles through the shadow boundary.' },
      { type: 'qa', question: 'LDS vs Apex for data?', answer: 'Use LDS (getRecord) for single-record CRUD to leverage the shared client-side cache. Use Apex for complex queries, multi-object DML, or business logic.' }
    ]
  },
  'salary': {
    title: 'Salary & Negotiation Strategy',
    subtitle: 'Scripts and benchmarks for Senior Salesforce roles',
    blocks: [
      { type: 'section', title: '💰 Negotiation Scripts' },
      { type: 'qa', question: 'Initial Counter-Offer', answer: '"Thank you for the offer. Based on market rates for senior Salesforce devs with mortgage expertise, I was expecting closer to [Target]. Is there flexibility?"' },
      { type: 'qa', question: 'When they say "Best Offer"', answer: '"I appreciate that. Beyond base, can we discuss bonuses, certification sponsorship, or remote flexibility?"' }
    ]
  },
  'questions': {
    title: 'Questions to Ask Interviewers',
    subtitle: 'Reverse interviewing to vet the team and role',
    blocks: [
      { type: 'section', title: '🧐 Smart Questions' },
      { type: 'qa', question: 'Technical Culture', answer: '"What does your current architecture look like? How do you handle technical debt vs new features?"' },
      { type: 'qa', question: 'Role Success', answer: '"What does success look like in the first 90 days? What is the biggest challenge the person in this role will face?"' }
    ]
  },
  'sc_objects': {
    title: 'Objects & Fields — Scenario Mix',
    subtitle: 'Data modeling, Relationships, and External IDs',
    blocks: [
      { type: 'section', title: '🏗️ Data Architecture' },
      { type: 'qa', question: 'Lookup vs Master-Detail?', answer: 'Master-Detail: Child deleted when parent deleted, rollup summaries allowed, parent mandatory. Lookup: Child stays when parent deleted, no rollups, parent optional. Use MDR for tight coupling (Loan to App).' },
      { type: 'qa', question: 'When to use External IDs?', answer: 'For integration sync. Allows upserting records using the external system’s ID (e.g., LOS_ID) without querying the Salesforce ID first. Faster and more reliable sync.' }
    ]
  },
  'sc_recordpage': {
    title: 'Record Page & UI Scenarios',
    subtitle: 'LWC, LDS, and LMS on standard pages',
    blocks: [
      { type: 'section', title: '💻 UI Integration' },
      { type: 'qa', question: 'How do sibling LWCs communicate on a page?', answer: 'Use Lightning Message Service (LMS). It is the modern replacement for Aura application events. Works across LWC, Aura, and Visualforce on the same page.' },
      { type: 'qa', question: 'LDS getRecord vs Apex?', answer: 'Use getRecord for reading single records to benefit from the shared client-side cache. If one component updates a field via LDS, all others using getRecord refresh automatically.' }
    ]
  },
  'comm30': {
    title: '30-Day Communication Plan',
    subtitle: 'Daily exercises to build English speaking confidence',
    blocks: [
      { type: 'section', title: '📅 Training Schedule' },
      { type: 'qa', question: 'Week 1: Foundations', answer: 'Focus on Pace and Clarity. Read aloud 30 mins daily. Eliminate "um/uh" by pausing instead. Record your self-intro and watch for filler words.' },
      { type: 'qa', question: 'Week 2: Structure', answer: 'Master the PREP framework: Point -> Reason -> Example -> Point. Every technical answer must have a project connection.' }
    ]
  },
  'mistakes': {
    title: 'Common Communication Mistakes',
    subtitle: 'Patterns that signal lack of seniority',
    blocks: [
      { type: 'section', title: '⚠️ Red Flags' },
      { type: 'qa', question: 'Mistake: Starting with "So basically"', answer: 'Sounds unconfident. Replace with a 1-second pause and a direct statement: "Governor limits are..."' },
      { type: 'qa', question: 'Mistake: Saying "We" instead of "I"', answer: 'Interviewers hire individuals. Own your decisions: "I designed the integration" instead of "We did it."' }
    ]
  },
  'mock': {
    title: 'Mock Interview Scripts',
    subtitle: 'Simulated rounds for technical and senior roles',
    blocks: [
      { type: 'section', title: '🎙️ Mock Rounds' },
      { type: 'qa', question: 'Round 1: Technical Core', answer: 'Focus on Apex, LWC, and Security. Practice answering "Tell me about yourself" in under 2 minutes.' },
      { type: 'qa', question: 'Round 2: Architecture', answer: 'Focus on Integration patterns, Design patterns, and Scenario-based modeling (e.g., Designing a notification system).' }
    ]
  },
  'fde_ag_concept': {
    title: 'Agentforce Architecture',
    subtitle: 'Autonomous AI Agents for Salesforce',
    blocks: [
      { type: 'section', title: '🤖 Core Components' },
      { type: 'qa', question: 'What is an Agentforce Agent?', answer: 'An autonomous AI entity with a defined Role, Goals, and Instructions. Unlike bots, agents use reasoning to decide which Actions to take to fulfill a user request.' },
      { type: 'qa', question: 'Agentforce vs Einstein Copilot?', answer: 'Einstein Copilot was a human-in-the-loop assistant. Agentforce is the next evolution: autonomous agents that can trigger on events, schedules, or chat.' }
    ]
  },
  'fde_dc_concept': {
    title: 'Data Cloud Fundamentals',
    subtitle: 'Unified Profiles and Real-time Data',
    blocks: [
      { type: 'section', title: '☁️ Data Architecture' },
      { type: 'qa', question: 'What is Unified Profile?', answer: 'Data Cloud ingests data from multiple sources (CRM, Marketing, Web) and uses Identity Resolution rules to link them into a single "Golden Record" for a customer.' },
      { type: 'qa', question: 'What is Grounding?', answer: 'Providing LLMs with real-time, company-specific data from Data Cloud to ensure AI responses are accurate and relevant to the business context.' }
    ]
  },
  'flows_guide': {
    title: 'Salesforce Flow — Master Guide',
    subtitle: 'Record-Triggered, Screen, and Schedule Flows',
    blocks: [
      { type: 'section', title: '⚙️ Automation Patterns' },
      { type: 'qa', question: 'Before-Save vs After-Save Flow?', answer: 'Before-Save: 10x faster, use for simple field updates on the same record. After-Save: Use for related records, emails, or async callouts.' },
      { type: 'qa', question: 'Flow vs Apex Trigger?', answer: 'Use Flow for simple logic and admin maintainability. Use Apex for complex bulk processing (>200 records), multi-object maps, or advanced error handling.' }
    ]
  },
  'reports_guide': {
    title: 'Reports & Dashboards Master',
    subtitle: 'Analytics, Joined Reports, and Analytics API',
    blocks: [
      { type: 'section', title: '📊 Analytics Strategy' },
      { type: 'qa', question: 'Dynamic Dashboard vs Standard?', answer: 'Standard: Everyone sees data as the "Running User". Dynamic: Each user sees data based on their own record access (e.g., my loans only).' },
      { type: 'qa', question: 'Apex Analytics API?', answer: 'Allows running reports programmatically via Reports.ReportManager. Use it to build custom UI charts or generate PDFs from report data.' }
    ]
  },
  'eng30': {
    title: '30-Day English Speaking Plan',
    subtitle: 'Daily exercises to build senior presence',
    blocks: [
      { type: 'section', title: '📅 Practice Routine' },
      { type: 'qa', question: 'Week 1: Foundations', answer: 'Focus on pace and clarity. Record a baseline self-intro. Read technical articles aloud for 30 minutes daily.' },
      { type: 'qa', question: 'Week 4: Polish', answer: 'Do 60-minute mock interview rehearsals. Focus on active listening and professional closing statements.' }
    ]
  },
  'eng_starters': {
    title: '50 Sentence Starters',
    subtitle: 'Senior-level phrases for technical answers',
    blocks: [
      { type: 'section', title: '🎙️ Speech Patterns' },
      { type: 'qa', question: 'Starting an answer', answer: '"In my experience building mortgage systems...", "The way I think about this is...", "Let me walk you through how this works in practice..."' },
      { type: 'qa', question: 'Explaining tradeoffs', answer: '"It depends on the specific situation. Here is how I decide...", "Both approaches work, but the tradeoff is..."' }
    ]
  },
  'eng_phrases': {
    title: 'Handling Difficult Situations',
    subtitle: 'Scripts for when things get tough',
    blocks: [
      { type: 'section', title: '🛠️ Recovery Scripts' },
      { type: 'qa', question: 'When you don\'t know the answer', answer: '"I haven\'t used that specific feature in production. Let me reason through it: based on how X works, I would expect Y..."' },
      { type: 'qa', question: 'When salary is asked', answer: '"Before I give a number, could you share the budgeted range? I want to make sure we are aligned..."' }
    ]
  },
  'sc_navmixin': {
    title: 'NavigationMixin Scenarios',
    subtitle: 'Programmatic navigation in LWC',
    blocks: [
      { type: 'section', title: '🚀 Nav Patterns' },
      { type: 'qa', question: 'Navigate to New Record with defaults?', answer: 'Use NavigationMixin with standard__objectPage and encodeDefaultFieldValues for pre-populating fields like Loan Type or Amount.' },
      { type: 'qa', question: 'Generate URL without navigating?', answer: 'Use [NavigationMixin.GenerateUrl] to create a relative link. Prepend window.location.origin for a shareable absolute URL.' }
    ]
  }
};
