// Version: 2026-04-24-T1030 (Industrial Enrichment v1410)
// =============================================
const DASHBOARD_VERSION = "2026-04-24-T1630 (app.v1410.js - Production Dashboard Logic)";
console.log('%c 🚀 Dashboard Version: 2026-04-24-T1630 (EXTREME LOGGING v1410)', 'color: #3b82f6; font-weight: bold; font-size: 1.2rem;');
...
// --- DYNAMIC PAGE LOADING ENGINE (v1410 Modular) ---
async function ensurePageLoaded(pageId) {
    console.log(`%c 🔍 [LOADER] Checking if page is loaded: ${pageId}`, 'color: #a855f7; font-weight: bold;');
    const pageEl = document.getElementById(pageId);
    if (!pageEl) {
        console.error(`%c ❌ [LOADER] CRITICAL: Element not found in DOM: #${pageId}`, 'color: #ef4444; font-weight: bold;');
        return false;
    }
    
    // Audit current state
    const display = getComputedStyle(pageEl).display;
    const visibility = getComputedStyle(pageEl).visibility;
    const opacity = getComputedStyle(pageEl).opacity;
    const contentLen = pageEl.innerHTML.trim().length;
    
    console.log(`%c 📊 [LOADER] Page ${pageId} Current State:
    - Display: ${display}
    - Visibility: ${visibility}
    - Opacity: ${opacity}
    - Content Length: ${contentLen}
    - Parent: ${pageEl.parentElement ? pageEl.parentElement.id : 'NONE'}`, 'color: #6366f1;');
...
async function showPage(id) {
  console.log(`%c --- 🚀 [NAV START] Target: ${id} ---`, 'background: #1e293b; color: #3b82f6; padding: 5px 10px; border-radius: 5px; font-weight: bold;');
  
  // DOM Audit before nav
  const main = document.getElementById('main');
  if (main) {
      const mainStyle = getComputedStyle(main);
      console.log(`🏠 [NAV] #main current state: 
