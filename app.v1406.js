...
// =============================================
const DASHBOARD_VERSION = "2026-04-24-T1100 (v1406)";
...
async function showPage(id) {
  console.log(`🚀 [NAV] Navigating to: ${id}`);
  ...
  console.log(`🧹 [NAV] Hiding all pages...`);
  ...
  console.log(`📍 [NAV] Target element:`, page);
  
  if (!page && !topicConfig[id]) {
    console.error(`❌ [NAV] No element found for ID: ${id}`);
    return;
  }

  // 1. Try Industrial Topic Sync (TOPIC_DATA)
  console.log(`🧪 [NAV] Checking industrial content for: ${id}`);
  const isIndustrial = renderTopicContent(id);
  
  if (isIndustrial) {
    console.log(`💎 [NAV] Found industrial content. Showing topic_viewer.`);
    page = document.getElementById('topic_viewer');
  } else if (!page && topicConfig[id]) {
    // 2. Fallback to AI Generator
    console.log(`🤖 [NAV] No content found. Falling back to AI Generator.`);
    page = document.getElementById('topic_viewer');
  ...
  if (page) { 
    console.log(`✨ [NAV] Showing page: ${page.id}`);
