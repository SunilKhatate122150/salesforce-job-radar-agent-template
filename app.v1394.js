...
var TOPIC_DATA = {
  ...
  'mobigic_pwc': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'How to handle Regulatory Compliance (GDPR) in Salesforce?', answer: 'Use Individual object tracking, Shield Encryption for PII, and Data Masking in sandboxes.' }
    ]
  },
  'thenken_globus': {
  ...
  'sales_cloud': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'Lead Conversion Process?', answer: 'Converts a Lead into an Account, Contact, and optionally an Opportunity. Field mapping is controlled in Lead Settings.' }
    ]
  },
  'service_cloud': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'What are Entitlement Processes?', answer: 'Timelines that define the service level you provide to customers (e.g., response times based on SLA).' }
    ]
  },
  'experience_cloud': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'Guest User Security?', answer: 'Since 2020, guest users have limited access and cannot own records or have "View All Data". Use Guest User Sharing Rules.' }
    ]
  },

  // --- AGENTFORCE & FDE MASTERY ---
  'fde_ag_concept': {
    title: 'Agentforce Architecture',
    subtitle: 'Autonomous AI Agents for Salesforce',
    blocks: [
      { type: 'section', title: '🤖 Core Components' },
      { type: 'qa', question: 'What is an Agentforce Agent?', answer: 'An autonomous AI entity with a defined Role, Goals, and Instructions. Unlike bots, agents use reasoning to decide which Actions to take.' },
      { type: 'qa', question: 'Agentforce vs Einstein Copilot?', answer: 'Einstein Copilot was human-in-the-loop; Agentforce is autonomous and can trigger on events.' }
    ]
  },
  'fde_atlas': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'What is the Reasoning Engine?', answer: 'The brain of Agentforce that plans and executes actions based on metadata and user context.' }
    ]
  },
  'fde_trust': {
  ...
  'apex': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'What is Apex? What are its key features?', answer: 'Apex is a strongly typed, object-oriented programming language that allows developers to execute flow and transaction control statements on the Salesforce platform.' },
      { type: 'qa', question: 'Difference between Public, Private, Protected, and Global?', answer: 'Private: class only. Public: namespace only. Protected: subclasses. Global: all namespaces.' }
    ]
  },
  'soql': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'What is SOQL injection?', answer: 'A vulnerability where malicious input manipulates queries. Prevent with bind variables or String.escapeSingleQuotes().' }
    ]
  },
  'triggers': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'Why use a Trigger Handler pattern?', answer: 'Separates routing from logic, improves testability, and helps manage recursion.' }
    ]
  },
  'async': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'Future vs Queueable?', answer: 'Future: Primitive params only. Queueable: Supports sObjects and chaining.' }
    ]
  },
  'lwc': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'Decorators: @api, @track, @wire?', answer: '@api: Public property. @track: Deep reactivity. @wire: Reactive data provisioning.' }
    ]
  },
  'integration': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'What are Named Credentials?', answer: 'Secure storage for endpoints and auth details, abstracting them from code.' }
    ]
  },
  'security': {
    title: 'Security & Sharing Model',
    subtitle: 'OWD, FLS, and Record Level security rules.',
    blocks: [
      { type: 'section', title: '🛡️ Security Layers' },
      { type: 'qa', question: 'Difference between with sharing and without sharing?', answer: 'with sharing respects the user\'s sharing rules; without sharing runs in system mode.' }
    ]
  },
  'platform': {
    title: 'Platform Events',
    subtitle: 'Event-driven architecture on Salesforce.',
    blocks: [
      { type: 'section', title: '📡 Messaging' },
      { type: 'qa', question: 'What is the Streaming API?', answer: 'Used to receive real-time notifications of changes to data in Salesforce.' }
    ]
  },
  'design': {
    title: 'Design Patterns',
    subtitle: 'Scalable architecture patterns for Apex.',
    blocks: [
      { type: 'section', title: '🏛️ Patterns' },
      { type: 'qa', question: 'Explain the Singleton pattern.', answer: 'Ensures a class has only one instance and provides a global point of access to it.' }
    ]
  },
  ...
  'adv_apex': {
    ...
    subtitle: 'Senior-level performance and scaling.',
    blocks: [
      { type: 'section', title: '🚀 Scaling' },
      { type: 'qa', question: 'How to reduce CPU time limits?', answer: 'Optimize loops, use Maps for O(1) lookups, and move logic to async if possible.' }
    ]
  },
  'adv_lwc': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'Benefit of Lightning Message Service?', answer: 'Allows communication between components across different DOM hierarchies (LWC, Aura, VF).' }
    ]
  },
  'adv_intg': {
    title: 'Advanced Integration',
    subtitle: 'Retry logic and enterprise patterns.',
    blocks: [
      { type: 'section', title: '🌐 Integration' },
      { type: 'qa', question: 'How to handle timeouts in callouts?', answer: 'Use setTimeout() on the HTTP request and implement a retry mechanism for transient errors.' }
    ]
  },
  'admin': {
    title: 'Admin & Configuration',
    subtitle: 'Objects, Fields, and Flows.',
    blocks: [
      { type: 'section', title: '⚙️ Config' },
      { type: 'qa', question: 'Lookup vs Master-Detail?', answer: 'Master-Detail has strict ownership and rollup summaries; Lookup is loosely coupled.' }
    ]
  },
  'scenario': {
    ...
    blocks: [
      { type: 'section', title: '🧠 Problem Solving' },
      { type: 'qa', question: 'Scenario: Record not updating in trigger?', answer: 'Check for recursion guards or if the DML is being blocked by a validation rule.' }
    ]
  },
  ...
  'domain': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'Explain the Loan Origination Process (LOS).', answer: '1. Lead -> 2. Application -> 3. Processing -> 4. Underwriting -> 5. Closing -> 6. Funding.' }
    ]
  },
  'comm30': {
  ...
  'speaking': {
    ...
    subtitle: 'Pronunciation and pace exercises.',
    blocks: [
      { type: 'section', title: '🗣️ Drills' },
      { type: 'qa', question: 'The Power Pause?', answer: 'Taking a 2-second pause before answering to show confidence and plan your response.' }
    ]
  },
  'mistakes': {
    title: 'Common Mistakes',
    subtitle: 'Patterns to avoid in senior interviews.',
    blocks: [
      { type: 'section', title: '⚠️ Red Flags' },
      { type: 'qa', question: 'Using "We" instead of "I"?', answer: 'Interviewers hire individuals; own your specific contributions and decisions.' }
    ]
  },
  'mock': {
  ...
  'sc_objects': {
    title: 'Objects & Fields Scenarios',
    subtitle: 'Complex data modeling and schema design.',
    blocks: [
      { type: 'section', title: '🏗️ Data Scenarios' },
      { type: 'qa', question: 'Scenario: Change Master-Detail to Lookup?', answer: 'Delete all Rollup Summary fields on the master object first, then change the field type.' }
    ]
  },
  'sc_recordpage': {
    ...
    blocks: [
      { type: 'section', title: '💻 UI Patterns' },
      { type: 'qa', question: 'How to refresh a standard page after Apex update?', answer: 'Use notifyRecordUpdateAvailable() or eval("window.location.reload()") as a fallback.' }
    ]
  },
  'sc_navmixin': {
    ...
    blocks: [
      ...
      { type: 'qa', question: 'Navigate to New Record with defaults?', answer: 'Use NavigationMixin with standard__objectPage and encodeDefaultFieldValues.' }
    ]
  }
};
...
var topicConfig = {
  ...
  'platform': { name: 'Platform Events', recommended: 60, group: 'Technical' },
  ...
  'accenture': { name: 'Accenture Prep', recommended: 60, group: 'Company' },
  'mobigic_pwc': { name: 'Mobigic / PWC', recommended: 45, group: 'Company' },
  'thenken_globus': { name: 'Thenken Globus', recommended: 45, group: 'Company' },
  'arago': { name: 'Arago Technical', recommended: 60, group: 'Company' },
  'morgan_stanley': { name: 'Morgan Stanley', recommended: 60, group: 'Company' },
